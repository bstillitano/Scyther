...
public struct FeatureToggle {
    ...
    /// The UserDefaults key used to store the local override value for a flag named `name`.
    ///
    /// Generated by converting the name to lowercase, replacing spaces with underscores,
    /// and prefixing with `Scyther_toggler_local_value_`.
    ///
    /// Example: "Secret Feature" becomes "Scyther_toggler_local_value_secret_feature".
    ///
    /// This is the single source of truth for the key derivation, so it can be shared with
    /// thread-safe accessors such as ``FeatureFlags/localOverride(for:)``.
    ///
    /// - Parameter name: The flag name to derive a key for.
    /// - Returns: The UserDefaults key string.
    static func localValueKey(for name: String) -> String {
        return "Scyther_toggler_local_value_\(name.lowercased().replacingOccurences(of: [" "], with: "_"))"
    }

    /// The UserDefaults key for storing this toggle's local override value.
    ///
    /// - Complexity: O(*n*) where *n* is the length of the name.
    private var defaultsKey: String {
        return Self.localValueKey(for: name)
    }
