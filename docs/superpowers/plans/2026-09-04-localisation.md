...
2. **Write the fragment** `Scripts/localization/strings/<Module>.json` with every key and all twelve languages, following the Translation Rules and Glossary. Before adding a key, grep the other fragments (`grep -l '"<key>"' Scripts/localization/strings/*.json`); if an earlier module already owns it, reuse it at the call site and do not add it again, since the generator rejects duplicates. Interpolations: `Int` → `%lld`, `String` → `%@`. If a literal interpolates anything else (`Double`, `Float`, `CGFloat`, `Date`), format it into a `String` first in the view and interpolate that. Counted strings get `"plural": true`.
